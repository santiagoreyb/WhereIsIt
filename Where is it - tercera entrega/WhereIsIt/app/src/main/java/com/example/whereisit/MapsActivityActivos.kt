...
import com.example.whereisit.databinding.MapsActivityActivosBinding
...
class MapsActivityActivos : AppCompatActivity(), OnMapReadyCallback {
    ...
    private lateinit var binding: MapsActivityActivosBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = MapsActivityActivosBinding.inflate(layoutInflater)
        ...
        mLocationCallback = object : LocationCallback() {
            override fun onLocationResult(locationResult: LocationResult) {
                ...
                val activosRef = database.getReference("users/$uid/activos")

                activosRef.addValueEventListener(object : ValueEventListener {
                    override fun onDataChange(dataSnapshot: DataSnapshot) {
                        for (activoSnapshot in dataSnapshot.children) {
                            val activoId = activoSnapshot.getValue(String::class.java)
                            if (activoId != null) {
                                val activoRef = database.getReference("activos/$activoId")
                                activoRef.addListenerForSingleValueEvent(object : ValueEventListener {
                                    override fun onDataChange(snapshot: DataSnapshot) {
                                        ...
                                        if (activo != null && activo.isDisponible) {
                                            // Enviar los valores de latitud y longitud a la base de datos
                                            val myRef = database.getReference("activos/$activoId")
                                            myRef.child("latitud").setValue(latitude.toString())
                                            myRef.child("longitud").setValue(longitude.toString())
                                        }
                                    }
        ...
        val activosButtonListener:Button = findViewById(R.id.ActivosButtonID)
        activosButtonListener.setOnClickListener {
            val activosIntent = Intent(this,ListaActivosAdapterActivity::class.java)
            startActivity(activosIntent)
        }
